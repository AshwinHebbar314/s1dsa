...
ITEM rear(QUEUE *q) {
    // Checking if QUEUE is empty
    // Item at the rear of the QUEUE cannot be returned if the QUEUE is empty
    
    if (!isEmpty(q))
        return q->queue[q->rear_queue];
    else
        return -1;
}

ITEM rear_pop(QUEUE *q) {
    
    if (!isEmpty(q)) {
        int lastItem = q->queue[q->rear_queue];
        q->rear_queue = q->rear_queue - 1;
        return lastItem;
    }
    else
...
void display(QUEUE *q) {
    ...
    if (!isEmpty(q)) {
        printf("\n\t\tQUEUE\n");
        printf("\tFRONT -> ");
        for (i = q->front_queue; i <= q->rear_queue; i++)
            printf("%d ", q->queue[i]);
        printf("<- REAR\n");
    }
    else
