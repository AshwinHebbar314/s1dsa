...
ListNode *createNode(ITEM item)
{
    ...
    {
        printf("SUCCESS - Malloc successfully allocated memory for item %d!\n", item);
        temp->item = item;
        temp->next = NULL;
        return (temp);
    }
...
/******
 * 
 * Operations defined as part of Lab 10
 * 
******/

int replace(LinkedList *list, ITEM item, ITEM newItem)
{
...
int insertBefore(LinkedList *list, ITEM item, ITEM newItem)
{
    ListNode *currentNode = list->head;
    ListNode *previousNode = list->head;
    ListNode *newNode = createNode(newItem);

    if (currentNode == NULL) { // case where List is empty 
        ...
        return 1; // case where element is not found after chceking every node.

    }
}

int replaceAll(LinkedList *list, ITEM item_to_find, ITEM replacing_item) {

    ListNode* currentNode = list->head;
    int flag = 0;

    while(currentNode != NULL) {
        if(currentNode -> item == item_to_find) {
            currentNode -> item = replacing_item;
            flag = 1;
        }
        currentNode = currentNode -> next;
    }
